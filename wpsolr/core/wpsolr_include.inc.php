...
// Definitions
define( 'WPSOLR_PLUGIN_VERSION', '18.4' );
...
function solr_post_save_admin_notice() {
	if ( $out = get_transient( get_current_user_id() . 'error_solr_post_save_admin_notice' ) ) {
		...
		echo "<div class=\"error wpsolr_admin_notice_error\"><p>(WPSOLR) Error while indexing this post/page:<br><br>$out</p></div>";
	}
			...
			[
				'data' =>
					[
						...
						'redirect_search_home'               => apply_filters( WpSolrFilters::WPSOLR_FILTER_REDIRECT_SEARCH_HOME, '' ),
					],
			]
		),
