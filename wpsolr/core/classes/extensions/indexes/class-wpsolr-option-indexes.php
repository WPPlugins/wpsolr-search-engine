...
class WPSOLR_Option_Indexes extends WpSolrExtensions {
	...
	function migrate_data_from_v4_9() {
		...
		$old_options      = WPSOLR_Service_Container::getOption()->get_option( $old_options_name, false );
		...
		foreach (
			...
			if ( ! empty( $old_options[ 'solr_host' . $old_index_postfix ] ) ) {
				...
				if ( ( isset( $old_options['host_type'] ) ? $old_options['host_type'] : '' ) === $old_index['host_type'] ) {
					...
					$option_last_post_indexed = WPSOLR_Service_Container::getOption()->get_option( 'solr_last_post_date_indexed' . $old_index['post_fix'], null );
	...
	public function get_index_search_engine_name( $solr_index ) {
		...
		return WPSOLR_AbstractEngineClient::ENGINE_SOLR === $this->get_index_search_engine( $solr_index ) ? WPSOLR_AbstractEngineClient::ENGINE_SOLR_NAME : WPSOLR_AbstractEngineClient::ENGINE_ELASTICSEARCH_NAME;
	}
	 ...
	 */
	public function create_managed_index( $managed_solr_service_id, $index_type, $index_uuid, $index_name, $index_protocol, $index_host, $index_port, $index_path, $index_key, $index_secret ) {
		...
		$solr_indexes[ $solr_index_indice ]['index_engine']                  = WPSOLR_AbstractSearchClient::ENGINE_SOLR;
