 ...
 */
class WPSolrIndexElasticaClient extends WPSOLR_AbstractIndexClient {
	...
	protected function search_engine_client_extract_document_content( $file ) {
		...
		if ( ! $result->hasError() ) {
			$attached_document = $this->get_elastica_type_attachment()->getDocument( $this->WPSOLR_DOC_ID_ATTACHMENT, [ 'stored_fields' => 'file.content' ] );
