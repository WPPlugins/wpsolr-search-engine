...
use wpsolr\core\classes\engines\elastica\WPSolrIndexElasticaClient;
...
abstract class WPSOLR_AbstractIndexClient extends WPSOLR_AbstractEngineClient {
	 ...
	 *
	 * @return WPSOLR_IndexSolariumClient|WPSolrIndexElasticaClient
	 */
	...
	public function index_data( $batch_size = 100, $post = null, $is_debug_indexing = false, $is_only_exclude_ids = false ) {
		...
		$post_types = str_replace( ',', "','", WPSOLR_Service_Container::getOption()->get_option_index_post_types_str() );
		...
		while ( true ) {
			...
			for ( $idx = 0; $idx < $post_count; $idx ++ ) {
				...
				if ( 'attachment' !== $ids_array[ $idx ]['post_type'] ) {
					...
					$post_attachments = apply_filters( WpSolrFilters::WPSOLR_FILTER_GET_POST_ATTACHMENTS, [], $postid );
	...
	public function get_blacklisted_ids() {
		...
		$excluded_list_ids = empty( $this->solr_indexing_options['exclude_ids'] ) ? [] : explode( ',', $this->solr_indexing_options['exclude_ids'] );
