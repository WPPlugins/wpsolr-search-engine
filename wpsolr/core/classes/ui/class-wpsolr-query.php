...
class WPSOLR_Query extends \WP_Query {
	...
	public function get_nb_results_by_page() {
		return WPSOLR_Service_Container::getOption()->get_search_max_nb_results_by_page();
	}
	...
	public function get_wpsolr_sort() {
		...
		if ( empty( $this->wpsolr_sort ) ) {
			$this->wpsolr_sort = apply_filters( WpSolrFilters::WPSOLR_FILTER_DEFAULT_SORT, WPSOLR_Service_Container::getOption()->get_sortby_default(), $this );
		}
	...
	function get_posts() {
		...
		try {//return parent::get_posts();
			...
			if ( apply_filters( WpSolrFilters::WPSOLR_FILTER_IS_PARSE_QUERY, true ) ) {
				$this->parse_query();
			}
			...
			do_action( WpSolrFilters::WPSOLR_ACTION_POSTS_RESULTS, $this, $this->results_set );
