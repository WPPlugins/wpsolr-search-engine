...
class WPSOLR_Translate {
	...
	static public function translate_field( $translation_domain, $field_name, $field_value ) {
		...
		$result = apply_filters( WpSolrFilters::WPSOLR_FILTER_SEARCH_PAGE_FACET_NAME, $field_name );
		...
		if ( $result === $field_name ) {
			...
			if ( ! empty( $field_value ) ) {
				...
				$result = apply_filters( WpSolrFilters::WPSOLR_FILTER_TRANSLATION_STRING, $field_value,
					[
	...
	static public function translate_field_custom_field( $translation_domain, $field_name, $field_value ) {
		...
		$result = apply_filters( WpSolrFilters::WPSOLR_FILTER_SEARCH_PAGE_FACET_NAME, $field_name_without_str );
		...
		if ( $result === $field_name_without_str ) {
			...
			if ( ! empty( $field_value ) ) {
				...
				$result = apply_filters( WpSolrFilters::WPSOLR_FILTER_TRANSLATION_STRING, $result,
					[
						'domain' => $translation_domain,
						'name'   => $field_name,
						'text'   => $result,
